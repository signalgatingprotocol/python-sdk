"""Installed, deterministic proof of Signal Gating's alert-routing value."""
...
class IncidentAlert(Signal):
    """Alert used by the deterministic incident-triage fixture."""
...
class TriageResult:
    """Measured outcomes from one incident-triage run."""
    ...
    @property
    def handler_load_reduction_percent(self) -> float:
        """Return the percentage of alerts kept away from the handler."""
        return round((1 - self.admitted_alerts / self.total_alerts) * 100, 1)

    @property
    def preserved_all_critical_incidents(self) -> bool:
        """Return whether every unique critical fingerprint was admitted."""
        return self.critical_incidents_preserved == self.unique_critical_incidents

    @property
    def matches_expected_fixture(self) -> bool:
        """Return whether the deterministic proof produced every expected metric."""
        return (
            self.total_alerts,
            self.admitted_alerts,
            self.priority_drops,
            self.duplicate_drops,
            self.unique_critical_incidents,
            self.critical_incidents_preserved,
        ) == (12, 3, 6, 3, 3, 3)
...
async def run_scenario() -> TriageResult:
    """Run the fixture through real SDK gates and return measured outcomes."""
...
def format_result(result: TriageResult) -> str:
    """Render stable, line-oriented metrics for humans and scripts."""
    return "\n".join(
        (
            f"signals_in={result.total_alerts}",
            f"signals_admitted={result.admitted_alerts}",
            f"handler_load_reduction={result.handler_load_reduction_percent:.1f}%",
            f"priority_drops={result.priority_drops}",
            f"duplicate_drops={result.duplicate_drops}",
...
def main() -> int:
    """Run and print the incident-triage proof."""
    result = asyncio.run(run_scenario())
    print(format_result(result))
    if result.matches_expected_fixture:
        print("result=PASS")
        return 0
